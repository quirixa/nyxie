...
// (not once per page like before), so the DOMContentLoaded listener
// above only fires on first load. Re-applying on every route change is
// handled by router.js itself (see render()), which calls applyTheme()
// and applyAccent() again after cloning each view's template — some
// elements (theme-preview cards, accent swatches) only exist once a
// given view is mounted and need the current value reflected right away.
