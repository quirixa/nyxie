...
const DEFAULT_ROLE_DEFS = [
  { key: 'admin', name: 'Admin', permissions: PERMISSIONS.ADMINISTRATOR, position: 100, is_default: 0 },
  {
    key: 'moderator',
    name: 'Moderator',
    permissions:
...
function createDefaultRoles(db, serverId, now) {
  ...
  for (const def of DEFAULT_ROLE_DEFS) {
    ...
    ids[def.key] = id;
    run(db, `
      INSERT INTO server_roles (id, server_id, name, permissions, position, is_default, created_at)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `, [id, serverId, def.name, def.permissions, def.position, def.is_default, now]);
  }
  return ids;
}
...
function getServerContext(db, serverId, userId) {
  ...
  const membership = get(db, `
    SELECT sm.server_id, sm.user_id, sm.joined_at, sm.role_id, sm.nickname, sm.muted,
           sr.name AS role_name, sr.permissions AS role_permissions, sr.position AS role_position
  ...
  return {
    ...
    // Owner implicitly outranks every role even if (unusually) they hold
    // no role row — treat missing role position as -1 so a real role
    // (position >= 0) still outranks a member with no role at all.
    rolePosition: membership.role_position != null ? membership.role_position : -1,
    permissions: membership.role_permissions || 0,
